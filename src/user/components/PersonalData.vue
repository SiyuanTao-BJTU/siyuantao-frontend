<script setup>
  ...
  const creditRateText = computed(() => {
    ...
    if (score === null || score === undefined) return '暂无信用分';
    // You can customize these texts or use el-rate's default behavior
    if (score >= 90) return `信用极好 (${score})`; 
    if (score >= 80) return `信用优秀 (${score})`;
    if (score >= 70) return `信用良好 (${score})`; // Added Fair for more granularity with 5 stars
    if (score >= 60) return `信用中等 (${score})`;
    return `信用较低 (${score})`;
  });
  ...
  const beforeAvatarUpload = (rawFile) => {
    ...
    if (!allowedTypes.includes(rawFile.type)) {
      ElMessage.error('头像图片必须是 JPG/PNG/GIF 格式!') // Add this to i18n: 'Avatar picture must be JPG/PNG/GIF format!'
      return false
    }
    if (rawFile.size / 1024 / 1024 > 2) {
      ElMessage.error('头像图片大小不能超过 2MB!') // Add this to i18n: 'Avatar picture size can not exceed 2MB!'
      return false
    }
    return true
  }
  ...
  const customAvatarUpload = async (options) => {
    ...
    try {
      ...
      if (response && response.data && response.data.url) { // Check for the nested data object and its url
        ...
        const errorMessage = (response && response.message) ? response.message : '头像上传失败';
      ...
      let errorMessage = '头像上传 API 请求失败';
...
// 组件全局函数定义
  ...
  const handleSave = () => {
    ...
    if (!username_res.valid){
      ElMessage.error("用户名长度应在1-150位之间")
      return;
    }
  ...
  function formatAuthStatus(statusValueFromBackend) {
    if (!statusValueFromBackend) return '未认证';
  ...
  const handleAuthSubmitted = () => {
    ElMessage.info('学生认证信息已提交，请等待审核');
    emits('updateSuccess'); 
  };
...
<template>
  <div class="personal-data-container">
    <div class="personal-data-title">
      ...
      <h2>{{ !isSearching ? '个人中心' : '查看用户资料' + (username ? ' - ' + username : '') }}</h2>
    </div>
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="用户名">
            <el-input v-model="modify_form.username" placeholder="请输入用户名"/>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="邮箱">
            <el-input v-model="modify_form.email" placeholder="请输入邮箱"/>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="联系方式">
            <el-input v-model="modify_form.contact" placeholder="请输入联系方式"/>
          </el-form-item>
        </el-col>
         <el-col :span="12">
            <el-form-item label="头像">
              ...
              <!-- <el-input v-model="modify_form.avatarUrl" placeholder="或输入头像URL" style="margin-top: 10px;"/> -->
              <!-- Fallback input for URL, maybe hide if uploader is preferred -->
            </el-form-item>
        </el-col>
      </el-row>
      <el-form-item label="个人简介">
        <el-input type="textarea" v-model="modify_form.bio" :rows="3" placeholder="请输入个人简介"/>
      </el-form-item>
    </el-form>
    ...
    <el-descriptions v-else :column="2" border class="profile-descriptions">
        <template #title>
            <div style="display: flex; align-items: center;">
                 ...
                 <span style="margin-left: 10px; font-size: 1.2em; color: #303133; font-weight: bold;">{{ origin_form.username || '未填写' }}</span>
            </div>
        </template>

        <el-descriptions-item label="用户名"><span class="description-value">{{ origin_form.username || '未填写' }}</span></el-descriptions-item>
        <el-descriptions-item label="邮箱"><span class="description-value">{{ origin_form.email || '未填写' }}</span></el-descriptions-item>
        <el-descriptions-item label="联系方式"><span class="description-value">{{ origin_form.contact || '未填写' }}</span></el-descriptions-item>
        <el-descriptions-item label="信用分" :span="origin_form.bio ? 1 : 2">
          <div style="display: flex; align-items: center; margin-top: 5px;">
            <el-rate
              ...
              :texts="['信用较低', '信用中等', '信用良好', '信用优秀', '信用极好']"
        ...
        <el-descriptions-item label="个人简介" :span="2" v-if="origin_form.bio"><span class="description-value">{{ origin_form.bio || '未填写' }}</span></el-descriptions-item>
    </el-descriptions>
    ...
    <div v-if="!isSearching" class="student-auth-section">
        ...
        <el-descriptions v-if="studentProfileData && studentProfileData.student_auth_status && studentProfileData.student_auth_status !== 'UNSUBMITTED'" :column="1" border class="student-auth-descriptions">
            <el-descriptions-item label="认证姓名"><span class="description-value">{{ studentProfileData.verified_real_name || '未填写' }}</span></el-descriptions-item>
            <el-descriptions-item label="认证学号"><span class="description-value">{{ studentProfileData.student_id || '未填写' }}</span></el-descriptions-item>
            <el-descriptions-item label="认证院系"><span class="description-value">{{ studentProfileData.verified_department || '未填写' }}</span></el-descriptions-item>
            <el-descriptions-item label="认证宿舍"><span class="description-value">{{ studentProfileData.verified_dormitory || '未填写' }}</span></el-descriptions-item>
...
.personal-data-container {
  ...
  border-radius: 8px;
  box-shadow: 0 2px 12px 0 rgba(0,0,0,0.08); /* Subtle shadow */
}
...
.personal-data-title {
  ...
  padding-bottom: 15px; /* Add padding below title */
  border-bottom: 1px solid #ebeef5; /* Subtle separator line */
}
...
.profile-descriptions {
  margin-top: 20px; /* Space above the descriptions */
  margin-bottom: 30px; /* Space below the descriptions */
}
...
.profile-descriptions :deep(.el-descriptions__content) {
     color: #606266; /* Medium grey text for content */
}

.description-value {
    color: #606266; /* Ensure consistent text color for values */
}

.student-auth-section {
  margin-top: 30px; /* Space above this section */
  padding-top: 20px; /* Padding inside this section */
  border-top: 1px solid #ebeef5; /* Subtle separator line */
}
...
.student-auth-descriptions :deep(.el-descriptions__label) {
    font-weight: bold; /* Bold labels */
    color: #303133; /* Dark text for labels */
}

.student-auth-descriptions :deep(.el-descriptions__content) {
    color: #606266; /* Medium grey text for content */
}
...
.calendar-area h3, .comments-area h3 {
  font-size: 1.3em;
  color: #303133; /* Dark text for section titles */
  font-weight: bold; /* Bold font */
  margin-bottom: 15px;
  text-align: center;
}
...
.comment-username {
  font-weight: bold;
  color: #4A90E2; /* Use accent color for username */
  cursor: pointer;
  margin-right: 10px;
}
...
.comment-time {
  font-size: 0.85em;
  color: #909399; /* Light grey text for time */
}

.comment-body p {
  margin: 0;
  color: #606266; /* Medium grey text for comment content */
  word-wrap: break-word;
}

.no-comments p {
  text-align: center;
  color: #909399; /* Light grey text for empty state */
  padding: 20px;
}
